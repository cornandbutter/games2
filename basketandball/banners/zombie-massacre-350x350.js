...
a.href = 'https://supagamez.com/games/zombie-massacre/';
...
var im = new Image();
im.src = 'banners/zombie-massacre-320x320.jpg';
...
{
	im.style.width = 350 * scale + 'px';

	tim.style.top = timTop * scale + "px";
	tim.style.left = timLeft * scale +  "px";
	tim.style.width = 156 * scale + 'px';
	
	var yOff = -29 * Math.abs(Math.sin( totalTime * Math.PI * 0.5 )) * scale;
	var xOff = 0;
	var ang = -2 * Math.sin( totalTime * Math.PI * 2.0 );
	
	tim.style.transform = 'rotate(' + ang + 'deg) translate(' + xOff + 'px,' + yOff + 'px)';
